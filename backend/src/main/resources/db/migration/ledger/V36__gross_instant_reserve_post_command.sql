-- owner: ledger — ADR-N11
-- One explicit ledger command, invoked only through LedgerPort's transaction-bound adapter.

GRANT USAGE ON SCHEMA ledger TO ledger_gross_instant_function_owner, gross_instant_executor_role;
GRANT SELECT, INSERT ON ledger.journal_entries, ledger.journal_lines TO ledger_gross_instant_function_owner;
GRANT SELECT, INSERT, UPDATE ON ledger.reservations TO ledger_gross_instant_function_owner;
GRANT SELECT, UPDATE ON ledger.liquidity_accounts TO ledger_gross_instant_function_owner;

CREATE FUNCTION ledger.gross_instant_reserve_post(
    p_tenant_id uuid,
    p_settlement_attempt_id uuid,
    p_payment_id uuid,
    ...
    p_currency char(3),
    p_command_id uuid,
    p_occurred_at timestamptz(3)
) RETURNS TABLE (
    outcome text,
    reservation_id uuid,
    terminal_entry_id uuid,
    terminal_occurred_at timestamptz(3),
    replayed boolean
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    SELECT * INTO v_debtor FROM ledger.liquidity_accounts
      WHERE id = p_debtor_account_id AND tenant_id = p_tenant_id FOR UPDATE;
    SELECT * INTO v_creditor FROM ledger.liquidity_accounts
      WHERE id = p_creditor_account_id AND tenant_id = p_tenant_id FOR UPDATE;
    IF NOT FOUND OR v_debtor.id IS NULL OR v_creditor.id IS NULL THEN
        RAISE EXCEPTION 'gross instant account is missing or belongs to another tenant' USING ERRCODE = '42501';
    END IF;
    SELECT * INTO v_reservation FROM ledger.reservations
      WHERE settlement_attempt_id = p_settlement_attempt_id FOR UPDATE;
    IF FOUND THEN
        RAISE EXCEPTION 'gross instant concurrent attempt must replay after retry' USING ERRCODE = '40001';
    END IF;
    ...
    VALUES (gen_random_uuid(), p_settlement_attempt_id, p_payment_id, p_debtor_account_id, p_amount_minor,
        p_currency, 'ACTIVE', v_reserve_entry)
