...
/**
 * EPIC-27 Story 27.2C: the single atomic unit tying Story 27.2B's pure policy to persistence and
 * event publication — one transaction: run {@link Pacs002CorrelationPolicy}, persist the decision
 * to {@code iso.iso_message_correlation} (every outcome), and — {@code MATCHED} only — write an
 * {@code iso.outbox_events} row for {@code iso.message.correlated}. {@code iso-adapter} correlates
 * only; it never mutates {@code payment.*} or calls into {@code payment-lifecycle}'s FSM (root
 * {@code AGENTS.md} binding rule) — that consumption is a separate capability (blocked
 * {@code EPIC-20} Story 20.3), not built here.
 */
@Service
public class Pacs002CorrelationService {
    ...
    private String serialize(IsoMessageCorrelatedEvent event) {
        try {
            return objectMapper.writeValueAsString(event);
        } catch (RuntimeException exception) {
            throw new IllegalStateException("Could not serialize iso.message.correlated event", exception);
        }
    }
}
