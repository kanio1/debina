...
import type { PaymentAuditTrailQuery, PaymentIsoEvidenceQuery } from "@/generated/graphql";
type State = "idle" | "loading" | "ready" | "empty" | "unauthorized" | "error";
type Entry = PaymentAuditTrailQuery["paymentAuditTrail"]["items"][number];
type IsoEvidence = PaymentIsoEvidenceQuery["paymentIsoEvidence"];
...
export function AuditDrawer({ paymentId }: { paymentId: string }) {
 ...
 return <Sheet open={open} onOpenChange={setOpen}><SheetTrigger render={<Button variant="outline" data-testid="payment.detail.evidence.trigger" />}>Evidence</SheetTrigger><SheetContent data-testid="evidence.record.drawer"><SheetHeader><SheetTitle>Evidence / Audit</SheetTitle></SheetHeader><p aria-live="polite" data-testid="audit.correlation-id.copy-status" className="sr-only">{copyMessage}</p>{state==="loading"&&<ScreenState kind="loading" testIdPrefix="evidence.record" />}{state==="empty"&&<ScreenState kind="empty" testIdPrefix="evidence.record" message="No audit entries." />}{state==="unauthorized"&&<ScreenState kind="unauthorized" testIdPrefix="evidence.record" />}{state==="error"&&<ScreenState kind="error" testIdPrefix="evidence.record" message="Audit history is unavailable." />}{state==="ready"&&<div className="overflow-auto"><table data-testid="audit.trail.table" className="w-full text-left text-sm"><thead><tr><th>When</th><th>Actor</th><th>Action</th><th>Outcome</th></tr></thead><tbody>{items.map(e=><tr key={e.auditEntryId}><td>{new Date(e.occurredAt).toLocaleString()}</td><td>{e.actorId} · {e.authorizedRole}</td><td>{e.commandType}</td><td>{e.outcome}</td></tr>)}</tbody></table><table data-testid="audit.command-history.table" className="mt-4 w-full text-left text-sm"><thead><tr><th>Command</th><th>Decision</th><th>Correlation</th></tr></thead><tbody>{items.map(e=><tr key={e.auditEntryId}><td>{e.commandType}</td><td><details><summary>View decision state</summary>{e.beforeState.approvalStatus??"—"} → {e.afterState.approvalStatus??"—"}</details></td><td>{e.correlationId ? <><span className="select-text">{e.correlationId}</span><Button size="xs" variant="ghost" data-testid="audit.correlation-id.copy-button" aria-label="Copy correlation ID" onClick={()=>void copyCorrelationId(e.correlationId,setCopyMessage)}>Copy</Button></> : "—"}</td></tr>)}</tbody></table></div>}<IsoEvidenceSection open={open} paymentId={paymentId} /></SheetContent></Sheet>;
}

function IsoEvidenceSection({ open, paymentId }: { open: boolean; paymentId: string }) {
 const [state,setState]=useState<State>("idle"); const [evidence,setEvidence]=useState<IsoEvidence | null>(null);
 useEffect(()=>{if(!open)return;let active=true;queueMicrotask(()=>active&&setState("loading"));void fetch("/api/graphql",{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify({operationName:"PaymentIsoEvidence",variables:{paymentId}})}).then(async r=>{if(!active)return;if(r.status===401||r.status===403)return setState("unauthorized");if(!r.ok)return setState("error");const b=await r.json() as {data?:PaymentIsoEvidenceQuery;errors?:unknown[]};const value=b.data?.paymentIsoEvidence;if(b.errors?.length||!value)return setState("error");setEvidence(value);setState(value.messages.length||value.identifiers.length?"ready":"empty")}).catch(()=>active&&setState("error"));return()=>{active=false}},[open,paymentId]);
 return <section className="mt-6" aria-labelledby="iso-evidence-title"><h2 id="iso-evidence-title" className="text-sm font-medium">ISO messages and identifiers</h2>{state==="loading"&&<ScreenState kind="loading" testIdPrefix="evidence.record.iso" />}{state==="empty"&&<ScreenState kind="empty" testIdPrefix="evidence.record.iso" message="No ISO evidence recorded yet." />}{state==="unauthorized"&&<ScreenState kind="unauthorized" testIdPrefix="evidence.record.iso" />}{state==="error"&&<ScreenState kind="error" testIdPrefix="evidence.record.iso" message="ISO evidence is unavailable." />}{state==="ready"&&evidence&&<><table data-testid="evidence.record.iso-messages.table" className="mt-3 w-full text-left text-sm"><thead><tr><th>Message type</th><th>Version effective from</th><th>Lineage role</th></tr></thead><tbody>{evidence.messages.map(m=><tr key={`${m.isoMessageId}-${m.lineageRole}`} data-testid="evidence.record.iso-messages.entry"><td data-testid="evidence.record.iso-message.type">{m.messageType}</td><td data-testid="evidence.record.iso-message.version">{m.versionEffectiveFrom??"—"}</td><td data-testid="evidence.record.iso-lineage-role">{m.lineageRole}</td></tr>)}</tbody></table><table data-testid="evidence.record.iso-identifiers.table" className="mt-4 w-full text-left text-sm"><thead><tr><th>Type</th><th>Value</th></tr></thead><tbody>{evidence.identifiers.map(i=><tr key={`${i.isoMessageId}-${i.type}`}><td data-testid="evidence.record.iso-identifier.type">{i.type}</td><td data-testid="evidence.record.iso-identifier.value" className="select-text">{i.value||"—"}</td></tr>)}</tbody></table></>}</section>;
}
