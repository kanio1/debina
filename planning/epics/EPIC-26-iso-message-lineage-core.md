...
`[PLANNING-DEFECT 2026-07-14]`: wybrano ten epik priorytetowo (przed innymi nowymi kandydatami) zgodnie z zasadą "dokończ epik, który faktycznie odblokuje story wcześniejszego epika `in-progress`" — `EPIC-20` Story 20.3 wymienia `EPIC-26` w `depends_on`. Story 26.1/26.2 okazały się **już w pełni zbudowane** przez `EPIC-19` Story 19.1 (te same migracje `V11__iso_json_direct_lineage.sql` tworzą dokładnie `iso.iso_messages`/`iso.iso_message_versions`/`iso.message_lineage`) — potwierdzone tu ponownie jako WŁASNY deliverable tego epika (nie duplikacja praktyki, tylko formalne domknięcie właściwego epika za pomocą już-istniejącego artefaktu). Stories 26.1–26.3 are now `done`; Story 26.4 remains `in-progress` (Wave 11 implementation and Checkpoint 2 proven; Checkpoint 3 and remaining runtime proofs open — see Story 26.4 and the Wave 11 program).

`[STATUS-HYGIENE 2026-07-28]`: epic-level `done` was incorrect while Story 26.4 was unfinished. Epic status is `in-progress` until Story 26.4 reaches formal `done` under the Wave 11 remaining-proof rules. Epic `done` is not runtime evidence.

**Ważne dla `EPIC-20` Story 20.3**: mimo że Stories 26.1–26.3 dostarczyły rdzeń lineage, **Story 20.3 pozostaje zależna od rzeczywistej zdolności korelacji** — silnik korelacji 9-krokowy i realny kanał inbound status (`csm.response.received`/pacs.002) należą do `EPIC-27` oraz do modułu `simulation` (`EPIC-17`). Zgodnie z zasadą "story można odblokować wyłącznie gdy brakujący element istnieje rzeczywiście w kodzie" — nie odblokowuj Story 20.3 tylko dlatego, że część EPIC-26 jest już zaimplementowana.
...
`[HISTORICAL 2026-07-14 — superseded]`: At planning time this note claimed GraphQL did not exist in the repo and marked the story `blocked`. That current-state claim is false after Waves 9/10 delivered Query-only GraphQL. Keep only as history; do not treat as present status.

`[H8, 2026-07-16 — dual-agent governance/backlog-redesign session — partly historical]`: this story's deliverable (a read-model/GraphQL/frontend-facing lineage panel) differs from sibling lineage-write stories 26.1–26.3 — noted, not moved. The earlier `[OPEN-QUESTION]` that no epic owned building the first GraphQL schema/resolver is superseded as a *current* blocker for this story: Query-only GraphQL and `paymentIsoEvidence` now exist. Ownership-decision document hygiene remains outside this story's formal close criteria.

`[WAVE-11 EVIDENCE 2026-07-21 / hygiene 2026-07-28]`: Wave 11 delivered the ISO-owned typed read port, Query-only GraphQL field, fixed BFF operation and independent drawer section. Focused tests and Checkpoint 2 live Keycloak+BFF+Spring+isolated PostgreSQL proved JSON_DIRECT and signed pain.001, roles, tenant/branch isolation and honest optional identifiers. Checkpoint 3 is pending; remaining runtime proofs (two full backend regressions, governance validators, database review/cleanup) remain open. Formal story `done` is therefore not reached — status stays `in-progress`. A later Dagger Phase D figure `540/540` in `docs/ci/DAGGER-IMPLEMENTATION.md` (D2B, 2026-07-22) is out-of-scope Dagger regression evidence and is not Wave 11 Checkpoint 3. Record: `planning/programs/DEBINA-ISO-LINEAGE-IDENTIFIER-EVIDENCE-WAVE-11.md`.

Taski:
- [x] **Read model GraphQL: timeline lineage + panel identyfikatorów na szczególe płatności.**
      `verify: ./mvnw -f backend test -Dtest=PaymentLineageGraphQLTest` — PASS (implementation + focused verify). Live Checkpoint 2 evidence is in the Wave 11 record; Checkpoint 3 / remaining full-regression proofs remain open — do not treat this checkbox alone as formal Wave 11 completion.
