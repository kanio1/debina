...
**Status:** `TOOLING-BLOCKED-CHECKPOINT` (2026-07-22). **Baseline:** `676d09e19393830936f5bf419b40f4a5eaa2a4c3`. **Branch:** `rebase/enterprise-evolution`.
...
- Recorded the toolchain gate in `docs/ci/DAGGER-TOOLCHAIN-BASELINE.{md,yaml}` and the capped ADR-N16 smoke inventory in `docs/ci/SMOKE-CAPABILITY-MATRIX.{md,yaml}`.
...
`dagger` and `go` are absent. Dagger's stable module layout, generated bindings, engine declaration and first-class `check` syntax are version-sensitive, so no guessed module or bindings were created. Podman `5.8.4` is rootless; Dagger's official Podman guidance requires rootful execution and warns about the Engine PID limit. No host runtime/security setting was changed.
...
The workstation owner must install Dagger `v0.21.4`, provide a compatible Go toolchain and satisfy the official rootful-Podman/PID prerequisite. Then begin with the minimal engine probe, initialize the one Go module through that installed CLI, and execute the Phase D implementation/validation sequence. Wave 12 at `5ebebb0` remains untouched; no production payment code, migration, GraphQL schema, runtime compose, realm source, workflow, remote CI or `act` was changed.
