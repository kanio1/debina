...
|---|---|---|---|---|
| D0 credential authority | D3A proof; `dagger/credentials.go`, `smoke.go`, `integration.go`, pure tests | create one typed bundle; replace plaintext Dagger password env/Maven args; preserve authorities; `go test ./...`, `dagger call fast` | `SECRET_AUTHORITY`, 5m | only Dagger files; `refactor(ci): centralize phase d credential authority`; revert one commit; DONE only on proof |
...
| D5 smoke aggregation | D2–D4; `checks.go`, manifest/docs | add `SmokeAuth`, `SmokePayments`; classify their complete sequential aggregate as `SmokeSuite`; retain `Smoke` only as a deprecated alias | `CHECK_COMPOSITION`, 10m | aggregation files only; `feat(ci): compose phase d payment smoke checks`; revert D5; DONE |
...
| D9 final aggregate | D5–D8; `checks.go`, manifest/docs | `Acceptance` runs Fast/Integration in parallel and then SmokeSuite; PipelineAssurance remains independent, `PhaseD` remains an alias and BackendTestcontainers stays separate; prove focused gates then unfiltered `dagger check` | `AGGREGATE_DUPLICATION`, 20m | aggregate only; `feat(ci): add phase d socket free aggregate`; revert D9; DONE |
...
| D11 architecture hardening | D10; Dagger/docs plus focused null-safe read fix | independently audit check discovery, Workspace inputs, locks, Engine, caches, services, generators, traces and log health; prove `Acceptance` as the sole automatic check, staged execution, independent `PipelineAssurance`, callable compatibility aliases and preserved Phase D boundaries | `DUPLICATE_CHECK`, `STALE_WORKSPACE`, `UNPINNED_LOOKUP`, `UNEXPECTED_RUNTIME_ERROR`; measured | logical local commits only; no remote CI/Phase E; see dated architecture review; DONE |
