...
| D11 architecture hardening | D10; Dagger/docs plus focused null-safe read fix | independently audit check discovery, Workspace inputs, locks, Engine, caches, services, generators, traces and log health; prove one canonical unfiltered check and preserve Phase D boundaries | `DUPLICATE_CHECK`, `STALE_WORKSPACE`, `UNPINNED_LOOKUP`, `UNEXPECTED_RUNTIME_ERROR`; measured | logical local commits only; no remote CI/Phase E; see dated architecture review; DONE after final matrix |
