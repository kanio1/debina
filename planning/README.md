...
16. **[OPEN-QUESTION 2026-07-15] EPIC-10 — koordynacja transakcji jeden-writer-per-schema między `payment-lifecycle` a `iso-adapter`**: `PaymentService.submitPayment` (i analogicznie `Pain001SubmissionEndpointTest`'s ścieżka) to dziś jedna `@Transactional` metoda pisząca w jednej transakcji JDBC zarówno do schematu `payment` (`payments`, `outbox_events`, `payment_status_history`, `payment_events`) jak i do schematu `iso` (`iso_messages`, `payment_iso_identifiers`, `message_lineage`) — działa dziś, bo obie grupy tabel mają wspólnego writera (`sepa_app`) w jednym deployowalnym procesie. Żaden przeczytany dokument (`sepa-nexus-blueprint-ownership-integration.md` §3.6/§9, `sepa-nexus-message-flow-and-data-blueprint.md` §8, ADR-N2) nie rozstrzyga, co ma się stać z tym wzorcem po `EPIC-10` Story 10.1 (gdy `iso-adapter` dostaje własną, odrębną rolę DB) — ADR-N2 rozstrzyga ten sam typ pytania tylko dla `routing` ("in-process Modulith module, no gRPC call"), nic analogicznego nie mówi dla `iso-adapter`. Dwie możliwe drogi (async outbox/inbox vs. jakiś mechanizm współdzielonej transakcji w ramach jednego procesu) obie są architektonicznie uzasadnione, ale wybór zmienia rzeczywiste gwarancje atomowości dzisiejszego kodu — nierozstrzygnięte samodzielnie, wymaga decyzji użytkownika przed rozpoczęciem `EPIC-10`. Pełny zapis: `planning/epics/EPIC-10-iso-lineage-ownership.md`.
