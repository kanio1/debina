...
| EPIC-27 — ISO: silnik korelacji | in-progress (Story 27.1 done 2026-07-15; **2026-07-16: Story 27.2 [9-step correlation] rozdzielona na 27.2A/27.2B/27.2C, wszystkie trzy `done`** — steps 1–4/6/7 zaimplementowane test-first z mutation-proof, migracja `V21` (`iso.iso_message_correlation`+`iso.iso_messages.tenant_id`+`iso.payment_iso_identifiers.tx_id`+`iso.outbox_events`), `iso.message.correlated` publikowany na `MATCHED`; step 5 (fallback) świadomie NIE zaimplementowany — `[OPEN-QUESTION]`, brak źródłowo poprawnej ścieżki danych; pełna regresja backendu `205/205 PASS`; Story 27.3 [duplicate/out-of-order] jest następnym krokiem, celowo nierozpoczętym w tej sesji; patrz epic file) | EPIC-26/Story 26.3 | [epics/EPIC-27-iso-correlation-engine.md](epics/EPIC-27-iso-correlation-engine.md) |
