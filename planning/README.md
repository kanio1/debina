...
13. **EPIC-19 Story 19.4 — dodatkowy, silniejszy powód blokady** (odkryte 2026-07-14): poza rozmiarem zadania (`CanonicalMapper`), zamrożona reguła z `CLAUDE.md` ("`signature` verification runs before ISO XML parsing — enforced ordering rule, not just documentation") oznacza, że **żaden zgodny z architekturą endpoint XML nie może dziś istnieć w ogóle** — `signature` moduł (EPIC-31) ma zero kodu, więc jakikolwiek endpoint parsujący XML bez weryfikacji podpisu naruszałby zamrożoną kolejność. To silniejszy, architektoniczny powód niż tylko "duże zadanie" — Story 19.4 jest transitywnie zależne od EPIC-31, nie tylko od Story 19.3.
14. **`PaymentService.visiblePayments`/`paymentDetail` — dostęp odczytowy ograniczony tylko do `payment_submitter`** (odkryte 2026-07-14, przy budowie EPIC-24 Story 24.2): §9 Role-to-Workspace Matrix przyznaje odczyt workspace'u 2 (Payments & Files) także `payment_viewer`/`payment_approver`/`operator`/`auditor`, ale `@PreAuthorize` na obu metodach dopuszcza wyłącznie `payment_submitter`. Nie naprawiono w tej sesji (zmiana reguły autoryzacji jest bezpieczeństwo-wrażliwa i poza wybranym zakresem 3 epików) — odnotowane jako otwarta rozbieżność do zaadresowania przy najbliższej pracy nad rolami/autoryzacją odczytu.
