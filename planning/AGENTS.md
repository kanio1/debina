...
Start from `HANDOFF.md`, then `planning/README.md` — an index, not an automatic ordering algorithm. Priority goes to the story classified analytically `READY` (see status vocabulary above), not to the first story carrying a formal `not-started`/`in-progress` frontmatter status. An `in-progress` story with no executable next task is a status defect, not a valid pick. Epic number does not determine implementation order. Never start a story that has an open decision gate; confirm its `verify:` is actually executable before coding.

Work story by story, task by task inside the epic file; tick a checkbox only once its `verify:` genuinely passes. Update `status` in the frontmatter whenever all of an epic's/story's tasks are ticked — checkbox state and `status` must stay consistent (skill `epic-story-task-catalog`). After any change to dependencies, regenerate and re-verify the capability graph (`planning/capabilities.yaml` / `planning/capability-graph.json` / `tools/agent-config/validate-capability-graph.py`). Overwrite `HANDOFF.md` before ending the session, every time.

For new or materially changed business work, apply `enterprise-use-case-engineering` and record a use-case slice, source trace, applicable rules, quality scenario, architecture realization and `semantic_enforcement: ENFORCED` marker before claiming `READY`. An active new or materially changed story must have `PRE_PLAN_READINESS: READY` before technical planning; formal epic/story status remains orthogonal to that analytical verdict. EPIC-76 is the C1 `PILOT`; legacy work migrates under `docs/governance/SEMANTIC-ENFORCEMENT-MIGRATION.md`.
