...
for name,item in active.items():
 if not (ROOT/item['path']/'SKILL.md').is_file(): errors.append(f'active registry entry missing skill: {name}')
...
print(f'REGISTRY: PASS ({len(active)} active, {sum(x["status"]=="PLANNED" for x in reg["skills"])} planned)')
