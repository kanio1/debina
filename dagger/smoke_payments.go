...
func (m *DebinaVerification) SmokeMakerCheckerApproval(ctx context.Context) (string, error) {
	...
	browser := m.paymentPlaywright(runtime, "test \"$(cat /tmp/phase-d-approval-rule)\" = 'PHASE-D approval matrix fixture verified' && pnpm exec playwright test e2e/d3b-maker-checker-approval.spec.ts --project=chromium --workers=1").
		WithFile(d3BApprovalRulePath, approvalRule)
	paymentID := browser.File(d3BPaymentIDPath)
...
func (m *DebinaVerification) paymentPlaywright(runtime *paymentSmokeRuntime, command string) *dagger.Container {
