...
`dagger check smoke` nie jest jeszcze potwierdzonym proofem D3A. Pierwotny leaf zatrzymał się przed Chromium, bo oddzielny `WithNewFile` marker nie wymuszał wykonania Flyway; to zostało rozdzielnie naprawione i potwierdzone przez credential oraz backend readiness. Dwa ograniczone leafe pionowe uruchomiły Chromium oraz realną stronę Keycloak, ale nie doszły do przesłania poświadczeń: pierwszy (`/tmp/debina-d3a-login-session-health-fixed.log`, `elapsed=1:15.91 exit=1`) nie znalazł `getByLabel(/^password$/i)`, a jeden zmieniony proof (`/tmp/debina-d3a-login-session-health-locator-fixed.log`, `elapsed=1:13.12 exit=1`) potwierdził brak `input[name="password"][type="password"]` przez 10 s. Następnie bezsekretny leaf diagnostyczny przeszedł (`/tmp/debina-d3a-keycloak-login-form-evidence.log`, `elapsed=1:06.72 exit=0`): domyślny flow realm nie ma własnego `browserFlow` ani `authenticationFlows`, a rzeczywista strona jest `USERNAME-FIRST-FORM` z polem `username` (accessible name `Username or email`) i przyciskiem `Sign In`. Zmieniony proof pionowy (`/tmp/debina-d3a-login-session-health-callback-fixed.log`, `elapsed=1:17.34 exit=1`) przesłał poświadczenia i zredagowany ślad dowiódł: Keycloak `302` wskazał dokładnie callback `http://frontend:3000/api/auth/callback`, request callbacku dotarł do frontendu, a callback zwrócił `307` z `Location: http://0.0.0.0:3000/`. Chromium zakończył na `chrome-error://chromewebdata/`, zanim doszło do `/payments`. Aktualna klasyfikacja to `POST-CALLBACK-REDIRECT-BLOCKED`; nie jest to DNS, połączenie do callbacku, locator ani odrzucenie credential. Odpowiedź callbacku osiągnęła końcowy redirect route, ale cookie, `/api/session` i shell nie są jeszcze browser-proven. Nie wolno ponawiać niezmienionego grafu ani zmieniać produkcyjnego callbacku bez osobnej decyzji. D3B i D4 nie zostały uruchomione. Należy nadal zachować user-owned `build/generated-spring-modulith/javadoc.json` poza stagingiem; zewnętrzny drift jest poza zakresem Phase D.

## Plan na następny krok

Wykonaj wyłącznie source-backed decyzję o naprawie post-callback redirectu: ustal, czy produkcyjny callback może zgodnie z kontraktem korzystać z już istniejącego publicznego `BFF_BASE_URL` zamiast transportowego `request.url`, albo uzyskaj osobną decyzję przed jakąkolwiek zmianą produkcyjnego kodu. Nie ponawiaj niezmienionego grafu, nie zmieniaj produktu ani bezpieczeństwa, nie używaj sekretów, localhost/workaroundów ani nie przechodź do podróży płatności.
