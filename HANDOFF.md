...
`dagger check smoke` nie jest jeszcze potwierdzonym proofem D3A. Pierwotny leaf zatrzymał się przed Chromium, bo oddzielny `WithNewFile` marker nie wymuszał wykonania Flyway; to zostało rozdzielnie naprawione i potwierdzone przez credential oraz backend readiness. Dwa ograniczone leafe pionowe uruchomiły Chromium oraz realną stronę Keycloak, ale nie doszły do przesłania poświadczeń: pierwszy (`/tmp/debina-d3a-login-session-health-fixed.log`, `elapsed=1:15.91 exit=1`) nie znalazł `getByLabel(/^password$/i)`, a jeden zmieniony proof (`/tmp/debina-d3a-login-session-health-locator-fixed.log`, `elapsed=1:13.12 exit=1`) potwierdził brak `input[name="password"][type="password"]` przez 10 s. Aktualna klasyfikacja brzmi `D3A-KEYCLOAK-LOGIN-FORM-LOCATOR-BLOCKED`: rzeczywista strona używa innego etapu lub autentykatora formularza. Callback, `sepa_session`, `/api/session` i shell nie były osiągnięte; nie jest to defect backendu, bazy, PKCE, odrzucenia credential ani produkcyjnej semantyki. Nie wolno ponawiać niezmienionego testu. D3B i D4 nie zostały uruchomione. Należy nadal zachować user-owned `build/generated-spring-modulith/javadoc.json` poza stagingiem; zewnętrzny drift jest poza zakresem Phase D.

## Plan na następny krok

Wykonaj wyłącznie `D3A-KEYCLOAK-LOGIN-FORM-EVIDENCE-AND-LOCATOR-CORRECTION`: uzyskaj bezpieczne, ograniczone metadane rzeczywistego formularza Keycloak (URL, tytuł, widoczne nazwy kontrolek oraz atrybuty tag/type/name/id bez wartości), a następnie wybierz jeden źródłowo potwierdzony locator. Nie używaj sekretów, localhost/workaroundów ani nie przechodź do podróży płatności.
...
- Nie myl rozwiązanego Dagger credential-contract z obecnym browser-auth blockerem. Dwa locatory hasła nie odnalazły kontrolki na prawdziwej stronie Keycloak; żaden nie przesłał credential ani nie dowiódł callbacku/sesji.
